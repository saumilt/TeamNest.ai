...
## Iteration 159 (Jun 2026) — Chat fixes: AI-stuck, multi-add members, multi-file, model checkboxes, WhatsApp bubbles — VERIFIED (backend 3/3 + web, 100%)
Five user-reported chat issues (web), all green (report iteration_158).
- **[CRITICAL] @ai stuck on "thinking" fixed** (`ai_service.py`, `services/ai_runtime.py`): every LLM call is now time-bounded so one slow/stalled provider (or a big attachment-laden prompt) can't hang the whole multi-model run. Added `AI_MODEL_TIMEOUT` (90s) per model via `asyncio.wait_for` in `call_model`, `AI_SYNTH_TIMEOUT` (45s) on `synthesize_answer` (falls back to best single answer), `asyncio.gather(return_exceptions=True)` in `ask_models_parallel`, and a `_post_ai_error` finalize safety-net that posts a visible error answer so the chat never stays stuck. (Attachment text was already capped at 24k chars.)
- **Add multiple members without the dialog closing** (`AddMemberDialog.jsx`): removed the `onOpenChange(false)` fired after each add; now tracks `addedIds`, shows an "Added ✓" state per person, and closes only on Done/X. Applied to both workspace-member rows and search results, and to the invite-by-email path (form resets, dialog stays open).
- **Multi-file attach** (`chat/ChatComposer.jsx` + `Chats.jsx`): added `multiple` to the file input and made `onFileChange` loop over all selected files (`uploadFile` already appends).
- **Model-selection checkboxes** (`chat/AiModelPicker.jsx`): the "Ask AI with…" popover now renders a checkbox list (`ai-model-checkbox-<key>`) instead of pill toggles; keeps the "at least one selected" rule, Rec badge, presets bar, and remember toggle. Model list unchanged (user chose keep all 6 + fast variants); refreshing versions later = edit `ai_composer/constants.js` ALL_MODELS + backend `MODEL_CONFIG`.
- **WhatsApp-style bubbles** (`ui-v2/Bubble.jsx`): own messages were already right-aligned but the sent color (#3A2E14) was nearly identical to received (#17171E). Changed the `sent` variant to a clearly amber-tinted bubble (`bg-amber-400/[0.18] border-amber-400/40`); received stays neutral gray — clear two-color distinction, own-right/others-left.
- Tests: `/app/backend/tests/test_iteration158_multimodel_ai_stuck.py` (3/3). Non-blocking: transient WS `InvalidStateError` while socket still CONNECTING (pre-existing client race).
...
- **Mobile Calls (LiveKit)** on Expo. Reused the existing backend `/api/calls` contract (POST `/api/calls/start` {chat_id, mode}, POST `/api/calls/{id}/join`, POST `/api/calls/{id}/end`, GET `/api/calls/by-chat/{chat_id}`) — no backend changes.
  ...
  - **Platform-guarded** so the Expo WEB preview keeps bundling: `src/components/call/CallScreen.tsx` (native LiveKit room UI — mic/camera toggles, participant tiles, Android screen-share, hang-up) is resolved on iOS/Android + for TS; `CallScreen.web.tsx` (placeholder) is resolved for the web bundle, so `@livekit/react-native` never reaches web. Verified: web preview bundles fine, placeholder renders, no crash.
  - Entry points in `chat/[id].tsx`: audio + video call buttons in the header (group/direct only, hidden for personal_ai); an in-chat LIVE "call started" card with a Join button + an "ended" variant (renderItem branch for `call_started`/`call_ended` messages); inline error banner on start failure. New full-screen route `app/call/[id].tsx` joins the room and hands creds to the platform CallScreen; leaving ends the call.
  - ⚠️ The native audio/video/screen-share media UI CANNOT be validated on Expo Go or the web preview — it requires an Emergent **Publish** native build. iOS screen-share (Broadcast Extension) is deferred; the screen-share control renders on Android only.
- **Avatar Reactions** (delight): new `src/components/QuickReactBar.tsx` — a header smiley trigger that expands an emoji row (❤️👍🎉😂🔥); tapping fires a reanimated floating burst and posts the emoji as a quick message so others see it. Chat-header group avatar now has a subtle reanimated press-scale.
- Tested: testing_agent mobile (iteration_132) PASS for all web-observable call flows + no regressions (chat send, view switch, group avatars, onboarding). Avatar Reactions self-verified via screenshots (quick-react posts + burst; call cards render).
- Ops: `/app/scripts/end_active_calls.py` ends any stray active demo calls and posts an ended card.
...
- **Sections built (all 7, approved after Section-3 checkpoint):** 1 Hero (thesis-led headline), 2 Problem/Fragmentation (broken-chain Slack/Zoom/Notion/ChatGPT), 3 Collective Intelligence Engine (radial hub-and-spoke: Persistent Memory core + Conversation/Reasoning/Capture/Action/Import spokes, SVG connectors, mobile vertical-stack fallback, annotated "Engine at work" product surface), 4 four Engine-surface feature blocks (alternating, reusing GroupChatMock/SixModelStrip/CallTranscriptMock/TaskCardMock in consistent BrowserFrame chrome), 5 single narrative Proof walkthrough (4-step horizontal stepper), 6 Social proof (honest quiet band + testimonial + trust chips, no fabricated logos — matches live site), 7 closing CTA "Start building your team's collective intelligence." New system-first nav (Product/How it works/Pricing/About). Recurring node-grid "engine" motif ties sections together.
- **Service worker fix (`public/service-worker.js` CACHE→v2, `src/index.js` register with `updateViaCache:'none'` + `reg.update()`)**: the old SW was serving a stale app shell so new routes (like `/v2`) 404'd → redirected to `/`. Now browsers self-update on next visit (prevents "I deployed but see the old site").
- Verified via screenshots (all 7 sections render, 4 proof steps, both feature surfaces, final CTA). Static marketing page — no backend. NOT yet promoted to `/`; awaiting user approval to swap.
...
- **Stop AI** (parity): while the AI is thinking (running `ai_question` placeholder with no matching `ai_answer`), an `ai-thinking-indicator` + `stop-ai-btn` appears. Backend `POST /api/chats/{id}/ai/stop` marks running threads `canceled` + soft-deletes the placeholder; `ai_runtime.handle_ai_command` has a cancel checkpoint after the model call AND `_finalize_research` re-checks (won't clobber `canceled` → complete, won't post a discarded answer). Client tracks a local `stoppedThreads` set so the indicator clears instantly (~<0.5s) even when WS is blocked. Verified in DB: canceled thread posts 0 answers.
- **AI document reading** (already backend-complete, re-verified end-to-end per user's re-prioritization): `@ai` extracts text from attached docs (pdf/docx/xlsx/pptx/csv/txt/json/md) via `file_extract.py` AND merges documents from EARLIER messages (`gather_recent_attachments`) so historical questions ("what was the codename in the memo I shared?") answer correctly. E2E confirmed both current + historical.
- **Regression fix**: `POST /chats/{id}/leave` lost its route decorator when `/read` was added in a prior session — re-registered (was 404/unrouted; frontend Leave-chat dialog now works).
- **Perf refactor**: `GET /api/chats` no longer does a per-chat N+1 (last-message `find_one` + unread `count_documents` per chat). Replaced with 2 bulk aggregations (latest message per chat via `$sort`+`$group $first`; unread counts via a per-chat-floor `$or` + `$group $sum`) → constant 3 queries regardless of chat count. Verified byte-for-byte parity (same chats, unread counts, last-message timestamps) against the old logic.
- Tested: backend pytest 7/7 (iteration_117) + testing_agent web+mobile parity (iteration_118) — all 3 reported UI bugs fixed and closed. No blockers.
...
### Iteration 122 (Jun 2026) — Latest AI models, combined default view, chat-invite emails fix
- **P0 — invited user got no email (root cause found)**: `POST /api/chats/{id}/invite-guest` and `.../invite-member` created a brand-new account with a one-time password but sent NO email (only returned the OTP for the inviter to share manually). Added `_email_new_invitee()` in `routes/chats.py` — brand-new chat invitees now receive the same Mailgun "set your password" invite email (7-day token via `mint_invite_link`) that workspace-level invites send. Verified end-to-end: `email_sent: true` + Mailgun accepted (backend log). NOTE: the earlier `POST /workspace/invite` email fix + this one exist in PREVIEW; user must **Publish/Deploy** to reach production (teamnest.ai). Ensure prod `PUBLIC_BACKEND_URL=https://teamnest.ai` so reset links resolve.
- **P1 — latest ChatGPT/Claude models**: upgraded `ai_service.MODEL_CONFIG` — `chatgpt` → `gpt-5.6-sol` ("ChatGPT 5.6"), `claude` → `claude-sonnet-5` ("Claude Sonnet 5"), added new premium `claude-opus` → `claude-opus-4-8` ("Claude Opus 4.8"), `gemini` stays `gemini-3.1-pro-preview`. Registered `claude-opus` in `billing.MODEL_CREDIT_COST` (45 cr), `routes/ai.PREMIUM_MODEL_KEYS`, `ai_runtime` premium set, and strengths/weaknesses profile. Updated `services/ai_employee_dispatcher.PROVIDER_MAP` to the same latest strings. Frontend/mobile pickers refreshed: `ai_composer/constants.js`, `mobile/src/aiModels.ts` (adds Claude Opus 4.8; fast tier relabeled GPT-5.4 mini / Gemini 3.5 Flash), plus `Knowledge.jsx` + `mobile/app/documents/index.tsx` ASK_MODELS and `MentionPopover.jsx` default-model copy. Verified all 4 premium models return live `real=True` via EMERGENT_LLM_KEY and `GET /api/ai/models`.
- **Default chat view → Combined**: web `Chats.jsx` + mobile `app/chat/[id].tsx` now default to `"combined"` (was `"human"`), still honoring a user's previously-saved per-chat view. Verified on both surfaces via screenshot.
- Regression test: `tests/test_iteration122_latest_models.py` (5/5 pass).

### Iteration 122b (Jun 2026) — Model "best for…" tooltips + resend-invite verify + deploy check
- **Model tooltips**: added a shared `hint` ("best for…") to each model in `frontend/src/components/ai_composer/constants.js` (ALL_MODELS) and `mobile/src/aiModels.ts` (AI_MODELS). Web `AiModelPicker.jsx` shows a live hover-driven caption (`data-testid="ai-model-hint"`) + native `title` on every chip; `ModelComparePicker.jsx` chips get `title`. Mobile `app/chat/[id].tsx` inline picker + `src/components/AiComposeModal.tsx` show a tap-to-preview caption. Verified on web: "ChatGPT 5.6 — Strategy & structured thinking".
- **Resend invite**: confirmed already implemented on web — `components/team/MembersTable.jsx` renders a one-tap **Resend** button (`member-resend-{id}`) for pending members (status invited OR must_change_password), wired to `POST /api/workspace/invite/{id}/resend` (Mailgun). Verified via `/team` screenshot + curl (`{ok: true}`). NOTE: mobile has no member-management screen, so resend is web-only.
- **Deploy readiness**: deployment_agent PASS — no hardcoded secrets/URLs/ports, env usage correct across backend/web/mobile. Emergent auto-updates prod env vars on Publish. Publish via the Emergent Publish button. For custom domain teamnest.ai, ensure prod `PUBLIC_BACKEND_URL=https://teamnest.ai` so emailed set-password/reset links resolve.

### Iteration 129 (Jun 2026) — Mobile Chats parity (Phase 1) + presets/tracker follow-ups
- **P0 fix**: mobile AI model picker crashed with `pickerHint is not defined` — added the missing `useState` in `app/chat/[id].tsx`. Verified by testing agent (picker opens, hint caption renders).
- **Pending Invites tracker** (web+mobile): roster shows amber "Invited · <time>" vs green "Joined ✅ · <when>"; `reset_password` (kind=invite) now sets `accepted_at`; `/workspace/members` returns accepted_at/joined_at/last_invite_sent_at.
- **Invite Analytics (4a, on-demand Mailgun)**: `GET /api/workspace/invite-analytics` (cached 60s) → Sent/Delivered/Opened/Failed badge per pending member on web `/team` + mobile Team screen. `mailgun_service.fetch_events()`.
- **Model Presets (3a, workspace-shared)**: new `routes/team_tools.py` — GET/POST/DELETE `/api/workspace/model-presets` (seeds Deep dive / Fast / Research). Preset bar in web AiModelPicker + mobile chat picker + Ask-AI modal (apply/save/delete). Verified both platforms.
- **Mobile New Chat sheet** (`src/components/NewChatSheet.tsx`): Development project, New group, New contact, Invite via SMS/WhatsApp, Find friends from contacts + member search. Contacts matched privacy-preserving via on-device SHA-256 (`expo-crypto`, pepper `teamnest.v1.contact-match`) → `POST /api/contacts/match {hashes}`. Added `expo-contacts` + app.json contacts plugin.
- **Mobile Chats screen** (`app/(tabs)/index.tsx` rewrite): New Chat (＋) button, search, filter chips (All/Direct/Groups/AI/Unread), workspace switcher (shown only for >1 workspace) via `/api/workspace/switch`, folder filter chips via `/api/folders`.
- **Mobile Team screen** (`app/team.tsx`) linked from You tab; re-provisioned test account os@radciti.com to a permanent password.
- Tests: `tests/test_iteration122_latest_models.py`, `tests/test_iteration128_mobile_chats.py`. Testing agent iteration 128 = PASS.

### Phase 2 — PENDING (LiveKit calls on mobile)
- Add audio+video+screen-share calls to mobile using LiveKit (backend already configured: LIVEKIT_URL/API_KEY/API_SECRET/SIP_URI). REQUIRES a native/dev build — `@livekit/react-native`+`react-native-webrtc` are native-only and won't run in Expo Go/web preview; must be platform-guarded so the web preview bundle keeps working. Not startable/testable in preview.

### Iteration 130 (Jun 2026) — Marketing homepage rebuild (broadened positioning)
- **Goal**: reposition the public site from Enterprise-only to an "AI Collaboration Workspace" for Individuals, Students, Teams & Businesses, preserving brand/dark theme/auth/app routes. Enterprise knowledge-continuity retained as a dedicated section, not the whole story.
- **New homepage** `pages/web/HomeV2.jsx` (routes `/` and `/v2`) rebuilt into 13 sections via reusable components in `components/web/home/`: `NavV2` (Product + Use Cases dropdowns, Individuals/Students/Business/AI Models/Pricing/Sign In, Start Free, mobile hamburger), `Hero` (new headline "Where people and AI think together." + workspace product mock w/ Human|Combined|AI toggle), `AudienceCards` (4 cards), `ProblemSection`, `MultiModelSection` (reuses `SixModelStrip`), `HumanAiSection` (interactive Human/Combined/AI toggle), `AudienceSections` (Personal/Student/Team/Business — student academic-integrity note; business preserves Role Intelligence/institutional memory/governance), `UseCasesSection` + `CapabilitiesSection`, `PlansSection` (5 audience plan cards, no invented prices — route to /signup, /pricing, /support), `FinalCTASection`, `FooterV2` (Product/Solutions/Resources/Company). CSS-only staggered entrance animation (`.tn-reveal` in App.css, respects prefers-reduced-motion). SEO via `SeoHelmet`.
- **Landing pages** (public): `/individuals`, `/students`, `/teams`, `/business`, `/ai-research`, `/multi-model-ai` — DRY config-driven `pages/web/audience/AudienceLanding.jsx` reusing home sections + per-page SEO. `/enterprise` intentionally NOT added (authenticated app route preserved); Enterprise links point to `/business`.
- **ROOT-CAUSE FIX (`lib/api.js`)**: the 401 axios interceptor hard-redirects to `/` unless the path is in `PUBLIC_PATHS`. New marketing routes (and pre-existing `/v2`, `/legacy`) were missing → deep-links/hard-loads bounced to home while client-nav worked. Added all new marketing routes + `/v2` + `/legacy` to the allowlist. Verified all 7 routes hard-load with correct H1s in an isolated context.
- **Fixes**: mobile menu + nav dropdown backgrounds were see-through (`bg-[var(--w-x)]/NN` opacity modifier on CSS-var colors didn't render opaque in TW 3.4) → switched to solid bg / opaque bg-layer; mobile menu now full-screen. Service worker hardened (cache v4 + `no-store` for HTML/JS/CSS so a new deploy never runs a stale route table).
- **Testing**: testing_agent iteration (web) PASSED all functional flows (hero CTAs, audience cards, interactive toggle, dropdowns, plan cards, existing routes) via client navigation; the sole blocker (hard-load redirect) was then root-caused and fixed + self-verified. No backend changes.
