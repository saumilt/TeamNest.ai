...
- **Read Receipts (web + mobile)**: own human messages (text/file/voice/task) show a tick — single grey ✓ = sent, double grey ✓✓ = delivered, amber ✓✓ = read by everyone. Personal-AI chats show none (no other humans). In GROUP chats the tick is tappable → a "Message info" / "Read by" sheet listing Read · Delivered · Sent members with timestamps. Live via WS `read`/`delivered` events + a read-state refresh in the 4s poll safety-net. Web `components/chat/ReadReceipt.jsx`; mobile `src/components/ReadReceipt.tsx`. Testids: `receipt-<msgId>` (data-state sent|delivered|read), `receipt-btn-<msgId>`, `read-by-<msgId>`, `read-by-person-<id>`.
- **Partial AI Streaming (backend)**: `ai_service.ask_models_parallel()` gained an `on_result` callback awaited as each model lands (concurrent tasks preserved order). `ai_runtime._make_partial_streamer()` (multi-model only) accumulates `{model_key,model_name,answer,real}` onto the running `ai_question` placeholder's `metadata.partials` (+`partial_done`/`partial_total`) and broadcasts `message_updated` per model. Single-model runs don't stream. Cancellation-aware.
- **Partial AI Streaming (web + mobile)**: while a multi-model `@ai` runs, a streaming card shows each selected model with a spinner → its answer as it arrives, then a "Synthesizing…" line, replaced by the final synthesized answer. Web `components/chat/StreamingAiBubble.jsx` (`streaming-<qId>`, `streaming-model-<key>`); mobile `StreamingBubble` in `app/chat/[id].tsx`. Bottom "AI is thinking" pill suppressed while a streaming card is visible.
- **Tested**: testing_agent iteration_159 — backend 7/7 pytest PASS (`tests/test_iteration159_receipts_partials.py`); web + mobile testids verified; personal-AI no-tick verified. Main agent additionally confirmed end-to-end via API: read-state/mark_read/delivered work, multi-model partials populate then final answer posts, and a two-user read-flip (amit sends → raj reads → amit's read-state shows raj.read_at ≥ msg time = amber). Note: models finish <2s in the demo workspace so the streaming card is only briefly visible; the picker defaults to a single recommended model (multi-select preserved through send — streaming triggers only when ≥2 models chosen, by design).
